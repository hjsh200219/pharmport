...
> 생성일: 2026-03-16
> **최종 수정: 2026-03-16 (Iteration 2 — Architect/Critic 피드백 반영)**
...
2. **GT 기반 캘리브레이션**: 모든 threshold는 기존 텍스트 GT(21,706건)로 캘리브레이션한다. ad-hoc 상수는 사용하지 않는다.
...
| **방법** | 약품명 sim ≥ 0.95 **AND** 성분 sim ≥ `ingr_mfr_exempt_thresh` 이면 제조사 channel 면제 |
| **threshold 결정** | GT에서 제조사 미달 건(`mfr_sim < mfr_thresh`)의 `ingr_sim` 분포를 구하고, 그 분포의 **p5**(하위 5%)를 `ingr_mfr_exempt_thresh`로 사용. `calibrate_channels()`와 동일한 패턴. |
| **근거** | 약품명과 성분이 모두 높은 유사도로 일치하면 제조사 불일치는 OEM/수입처 차이일 가능성 높음 |
| **예상 수확** | ~20-26건 |
| **위험도** | LOW — 이미 2개 strong signal 통과 |
| **검증** | GT 대조로 100% 정확도 확인 |
...
| **threshold 설정** | `name_sim ≥ 0.99`: `ingr_thresh = p5` (GT에서 재캘리브레이션), `name_sim ≥ 0.97`: `ingr_thresh = p3`, `name_sim < 0.97`: 기존 p1 유지 |
...
| **방법** | Prefix 일치를 "후보 고정"으로 사용 (embedding 대신 텍스트 기반 후보 생성). 해당 PI 후보에 대해 성분·제조사 multi-channel 검증. Prefix 중 가장 긴 공통 부분을 가진 PI를 우선 선택 |
...
| **검증** | GT 대조 + prefix 길이 분포 분석 + 수동 샘플 **300건** |

#### Phase 4. Top-K Soft Reciprocal (10,732건 대상) ← *기존 Phase 3에서 이동*

| 항목 | 내용 |
|------|------|
| **대상** | Reciprocal Match 실패 10,732건 중 Phase 3에서 미해결인 건 (forward Top-1 sim ≥ 0.90) |
| **방법** | Forward Top-3 후보를 생성하고, 각 후보에 대해 reverse Top-3 내에 해당 medicine이 있으면 "Soft Reciprocal"로 인정. 이후 성분·제조사 모두 기존 threshold 이상 통과 필수 |
| **ambiguity check (양방향)** | (a) 같은 medicine에 2+ PI가 Soft Reciprocal → 스킵, (b) **같은 PI에 2+ medicine이 Soft Reciprocal → 스킵** |
| **근거** | 약품명이 유사한 다수의 제품이 존재할 때 Top-1이 아닌 Top-2/3이 정답인 경우가 있음 |
| **예상 수확** | ~500-1,500건 |
| **위험도** | MEDIUM — Top-K로 candidate 풀이 넓어지므로 multi-channel 필터의 역할이 중요 |
| **검증** | GT 대조 + 무작위 **300건** 수동 검토 |
...
| **방법** | `sorted_ingredient_embedding` ↔ `sorted_성분명_embedding` 직접 cosine similarity. 매우 높은 threshold (≥ p1 from direct GT calibration) + 추가 검증 조건: (a) Top-1이 유일하게 높은 sim (2위와 gap ≥ 0.05), (b) Reciprocal check (터울주성분 → pharmport_medicine 역방향 Top-1 일치) |
...
| **검증** | GT에서 검증 가능한 subset 전수 검증 + 수동 검토 **500건** + 역방향 cross-check |
...
**Option A (단계적 회수)를 채택한다.** Phase 순서는 Easy First에 따라 Prefix → Top-K. Phase 5의 직접 경로는 Option B의 캘리브레이션 방식을 차용하여 GT 기반 cross-validation을 선행한다.
...
4. Phase 5(직접 경로)는 GT cross-validation을 통해 신뢰도를 확보한 후 적용
...
| 제안 | 판단 | 근거 |
|------|------|------|
| **계층적 검증 (2차 GT)** | **채택** | Phase 3-4에서 GT 검증 불가능한 건(GT 없는 매칭)에 대해, Phase 1-2의 확정 결과를 "2차 GT"로 활용하여 추가 cross-check. 구현 비용 낮고 검증 커버리지 확대. |
...
| `calibrate_channels` | 성분·제조사 threshold 캘리브레이션 |

v3에서 **독립 구현하는 것들:**

| 함수/모듈 | 역할 |
|----------|------|
| `calibrate_mfr_exempt_thresh()` | Phase 1: GT 중 mfr 미달 건의 ingr_sim 분포 → percentile 기반 threshold |
| `find_topk_reciprocal()` | Phase 4: Top-K Soft Reciprocal + 양방향 ambiguity check |
| `find_prefix_matches()` | Phase 3: Prefix Text Match 후보 생성 |
...
| **n=300** | **~1.0%** | **최대 1.0% 에러율 — Zero Error 원칙과 합치** |
| **n=500** | **~0.6%** | **최대 0.6% 에러율** |
...
-- Phase 5 rollback (별도 컬럼 방식 사용 시)
UPDATE pharmport_medicine
...
- [ ] dry-run 모드 기본 지원
- [ ] `match_ingredient_v3.py` 스켈레톤 작성: v2에서 공용 함수 import 확인
...
- [ ] `calibrate_mfr_exempt_thresh()` 구현: GT에서 `mfr_sim < mfr_thresh`인 건의 `ingr_sim` 분포 → p5를 `ingr_mfr_exempt_thresh`로 설정
- [ ] Phase 1 로직: `name_sim ≥ 0.95 AND ingr_sim ≥ ingr_mfr_exempt_thresh` → 제조사 면제
- [ ] GT 검증 결과 100% 정확도 확인
...
- [ ] Tiered threshold 로직: name_sim 구간별 성분 threshold 차등 적용
  - `name_sim ≥ 0.99`: `ingr_thresh = p5` (GT에서 재캘리브레이션)
  - `name_sim ≥ 0.97`: `ingr_thresh = p3`
  - `name_sim < 0.97`: 기존 p1 유지 (변경 없음)
- [ ] GT 검증 + Stratified 수동 샘플 **300건** 검토
...
- [ ] GT 검증 + Stratified 수동 샘플 **300건**
- [ ] match_method = `'v3_prefix'`

**수락 기준**: GT 검증 0% error, 회수 건수 ≥ 1,000
...
- [ ] 추가 조건: `name_sim ≥ 0.90 AND ingr_sim ≥ ingr_thresh AND mfr_sim ≥ mfr_thresh`
- [ ] **양방향 ambiguity check**:
  - (a) 같은 medicine에 2+ PI가 Soft Reciprocal → 스킵
  - (b) 같은 PI에 2+ medicine이 Soft Reciprocal → 스킵
- [ ] Phase 1-3에서 이미 매칭된 건은 제외
- [ ] GT 검증 + Stratified 무작위 **300건** 수동 검토
- [ ] match_method = `'v3_topk_reciprocal'`
- [ ] 2차 GT 검증: Phase 1-3 확정 결과를 보조 GT로 활용하여 cross-check

**수락 기준**: GT 검증 0% error, 회수 건수 ≥ 500
...
- [ ] **5a. 캘리브레이션**: 기존 GT 29,196건에서 직접 경로 sim 분포 파악. 정답 MIC에 대한 direct cosine sim 분포 → p1을 `direct_thresh`로 설정
- [ ] **5b. Cross-validation**: 기존 매칭 29,196건 전체에 대해 직접 경로를 실행하고, 기존 결과와 일치율 측정. **일치율 < 95%이면 직접 경로 폐기 또는 threshold 상향**
- [ ] **5c. 미매칭 적용**: Phase 1-4 후 남은 미매칭 건에 대해 직접 경로 실행
  - 조건: `direct_sim ≥ direct_thresh AND sim_gap ≥ 0.05 AND reciprocal(터울→pharmport 역방향 Top-1 일치)`
- [ ] **5d. 검증**: GT 대조 + Stratified 수동 검토 **500건**
- [ ] match_method = `'v3_direct'`
- [ ] 저장: 확정된 스키마에 따라 저장 (별도 컬럼 `direct_ingredient_code` 권장)

**수락 기준**: Cross-validation 일치율 ≥ 95%, GT 검증 0% error
...
1. ✅ 모든 Phase에서 GT 검증 Error Rate = 0%
...
- 모든 threshold는 GT percentile 기반 (ad-hoc 상수 금지)
- match_method 추적 가능
- dry-run 모드 우선 실행
...
| **2** | Phase 1 ingr_sim 0.7 → GT percentile 기반 | ad-hoc `0.7` → `calibrate_mfr_exempt_thresh()` 함수로 GT 중 mfr 미달 건의 ingr_sim p5 사용. `calibrate_channels()` 패턴과 일관성 확보. | Phase 1 조건, Task Flow |
| **3** | 수동 샘플 크기 상향 (n≥300/500) | Phase 2: 50→300, Phase 3: 100→300, Phase 4: 100→300, Phase 5: 200→500. n=300이면 95% CI 상한 ~1%로 Zero Error 원칙과 합치. | 모든 Phase 검증 절차, 수동 검토 프로토콜 신규 섹션 |
...
| **7** | Architect Synthesis 3건 판단 | 계층적 검증(2차 GT): **채택**, Stratified Sampling: **채택**, Gated Release: **후속 검토**. 각각 근거 명시. | Architect Synthesis 신규 섹션, Phase 4 Task Flow (2차 GT) |
